...
describe('TikTok proxy settings', () => {
  it('exposes dedicated proxy fields in the config schema', () => {
    expect(tiktokConfigSchema.fields.map(field => 'key' in field ? field.key : '')).toEqual(expect.arrayContaining([
      'proxy.switch',
      'proxy.host',
      'proxy.port',
      'proxy.protocol',
  ...
  it('renders dedicated proxy controls on the TikTok web config page', () => {
    const controls = TikTokWeb({
      tiktok: {
        switch: true,
        videoTool: true,
        priority: 800,
