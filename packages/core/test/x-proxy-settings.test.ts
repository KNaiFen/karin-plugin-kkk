...
describe('X proxy settings', () => {
  it('exposes dedicated proxy fields in the config schema', () => {
    expect(xConfigSchema.fields.map(field => 'key' in field ? field.key : '')).toEqual(expect.arrayContaining([
      'proxy.switch',
      'proxy.host',
      'proxy.port',
      'proxy.protocol',
  ...
  it('renders dedicated proxy controls on the X web config page', () => {
    const controls = XWeb({
      x: {
        switch: true,
