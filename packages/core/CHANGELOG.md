...
## [4.0.23](https://github.com/ikenxuan/karin-plugin-kkk/compare/v4.0.22...v4.0.23) (2026-07-05)

### ✨ 新功能

* 新增随包发布的 `kkk-auto-update` sidecar，可轮询 tgz 目录、自动执行 `pnpm add` 并在 PM2 环境下触发 Karin 重启

### 📝 文档更新

* 补充 PM2 + tgz 自动部署说明，明确 `node-karin` 内建 PM2 兼容路径与 sidecar 用法

## [4.0.0](https://github.com/ikenxuan/karin-plugin-kkk/compare/v3.0.0...v4.0.0) (2026-07-02)

### ⚠️ Breaking Changes

* 移除 `karin-plugin-kkk/template` 与 `karin-plugin-kkk/amagi` 子路径导出；`core` 不再对外转发内部渲染层与 `amagi` 包
* 新增 `template/server` 作为唯一 SSR 入口；`@ikenxuan/amagi` 改为真实运行时依赖，调用方不再通过 `karin-plugin-kkk/amagi` 间接访问

### ♻️ 代码重构

* 新增 `@kkk/template-contracts` 统一承载 `core` / `template` 共享渲染契约，`core` 改为只从 `template/server` 获取服务端渲染能力

### 📝 文档更新

* 更新贡献文档中的包分层约定，明确新的 `core` / `template` / `amagi` 依赖边界

## [3.0.0](https://github.com/ikenxuan/karin-plugin-kkk/compare/v2.32.41...v3.0.0) (2026-07-01)

### ⚠️ Breaking Changes

* 收敛插件平台 HTTP API 到 `/api/kkk/v1`，移除嵌入式 `amagi` HTTP 路由与独立 API 服务能力
* 删除配置项 `APIServer`、`APIServerPort`、`APIServerMount`

### ♻️ 代码重构

* `Register.ts` 不再挂载 `/amagi/api/*`，插件只保留 `/api/kkk/v1` 平台控制面

### 📝 文档更新

* 更新配置说明，明确插件仅支持 `/api/kkk/v1` 作为对外平台 API 面
