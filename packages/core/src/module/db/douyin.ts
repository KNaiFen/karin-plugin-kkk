...
export class DouyinDBBase {
  ...
  private static readonly AWEME_CACHES_TABLE_SQL = `
  ...
  private async migrate (): Promise<void> {
    try {
      await this.runInTransaction(async () => {
        await this.recoverHalfMigratedAwemeCaches()

        const hasPushType = await this.hasAwemeCachesPushType()
        const hasWrongForeignKey = await this.hasLegacyAwemeCachesForeignKey()
        ...
        if (!hasPushType || hasWrongForeignKey) {
          logger.info('[DouyinDB] 开始重建 AwemeCaches 标准表结构...')
          await this.rebuildAwemeCachesTable()
        }
      })
    } catch (error) {
  ...
  private async createTables (): Promise<void> {
    const queries = [
      ...
      // 创建作品缓存表
      DouyinDBBase.AWEME_CACHES_TABLE_SQL,
  ...
  private async runInTransaction (task: () => Promise<void>): Promise<void> {
    ...
    try {
      ...
      try {
        ...
        logger.error('[DouyinDB] 数据库回滚失败:', rollbackError)
      }
      throw error
    }
  }

  private async hasAwemeCachesPushType (): Promise<boolean> {
    try {
  ...
  private async hasLegacyAwemeCachesForeignKey (): Promise<boolean> {
    const tableInfo = await this.allQuery<{ sql: string }>(
      'SELECT sql FROM sqlite_master WHERE type = \'table\' AND name = \'AwemeCaches\''
    )
    return tableInfo.length > 0 && tableInfo[0].sql.includes('FOREIGN KEY (groupId) REFERENCES Groups(id)')
  }

  private async recoverHalfMigratedAwemeCaches (): Promise<void> {
    const tables = await this.allQuery<{ name: string }>(
      'SELECT name FROM sqlite_master WHERE type = \'table\' AND name IN (\'AwemeCaches\', \'AwemeCaches_old\')'
    )
    const hasAwemeCaches = tables.some(item => item.name === 'AwemeCaches')
    const hasAwemeCachesOld = tables.some(item => item.name === 'AwemeCaches_old')
    if (!hasAwemeCachesOld) return

    logger.warn('[DouyinDB] 检测到遗留 AwemeCaches_old，开始恢复半迁移状态...')
    if (hasAwemeCaches) {
      await this.runQuery('DROP TABLE IF EXISTS AwemeCaches_recovery_old')
      await this.runQuery('ALTER TABLE AwemeCaches RENAME TO AwemeCaches_recovery_old')
    }

    await this.runQuery(DouyinDBBase.AWEME_CACHES_TABLE_SQL)

    if (hasAwemeCaches) {
      await this.runQuery(`
        INSERT OR IGNORE INTO AwemeCaches (aweme_id, sec_uid, groupId, pushType, createdAt, updatedAt)
        SELECT aweme_id, sec_uid, groupId, COALESCE(pushType, 'post'), createdAt, updatedAt
        FROM AwemeCaches_recovery_old
      `)
      await this.runQuery('DROP TABLE AwemeCaches_recovery_old')
    }
  ...
  private async rebuildAwemeCachesTable (): Promise<void> {
    await this.runQuery('DROP TABLE IF EXISTS AwemeCaches_old')
    await this.runQuery('ALTER TABLE AwemeCaches RENAME TO AwemeCaches_old')
    await this.runQuery(DouyinDBBase.AWEME_CACHES_TABLE_SQL)
    await this.runQuery(`
      INSERT INTO AwemeCaches (aweme_id, sec_uid, groupId, pushType, createdAt, updatedAt)
      SELECT aweme_id, sec_uid, groupId, COALESCE(pushType, 'post'), createdAt, updatedAt
      FROM AwemeCaches_old
    `)
    await this.runQuery('DROP TABLE AwemeCaches_old')
  ...
  async getOrCreateBot (botId: string): Promise<Bot> {
    ...
    await this.runQuery(
      'INSERT OR IGNORE INTO Bots (id, createdAt, updatedAt) VALUES (?, ?, ?)',
      [botId, now, now]
    )

    const bot = await this.getQuery<Bot>('SELECT * FROM Bots WHERE id = ?', [botId])
    return bot || { id: botId, createdAt: now, updatedAt: now }
  }
  ...
  async getOrCreateGroup (groupId: string, botId: string): Promise<Group> {
    ...
    const group = await this.getQuery<Group>('SELECT * FROM Groups WHERE id = ? AND botId = ?', [groupId, botId])
    return group || { id: groupId, botId, createdAt: now, updatedAt: now }
  }
  ...
  async getOrCreateDouyinUser (sec_uid: string, short_id: string = '', remark: string = ''): Promise<DouyinUser> {
    const now = new Date().toISOString()
    await this.runQuery(
      'INSERT OR IGNORE INTO DouyinUsers (sec_uid, short_id, remark, living, filterMode, createdAt, updatedAt) VALUES (?, ?, ?, ?, ?, ?, ?)',
  ...
  async subscribeDouyinUser (groupId: string, botId: string, sec_uid: string, short_id: string = '', remark: string = ''): Promise<GroupUserSubscription> {
    ...
    const now = new Date().toISOString()
    await this.runQuery(
      'INSERT OR IGNORE INTO GroupUserSubscriptions (groupId, sec_uid, createdAt, updatedAt) VALUES (?, ?, ?, ?)',
      [groupId, sec_uid, now, now]
    )

    const subscription = await this.getQuery<GroupUserSubscription>(
      'SELECT * FROM GroupUserSubscriptions WHERE groupId = ? AND sec_uid = ?',
      [groupId, sec_uid]
    )

    return subscription || { groupId, sec_uid, createdAt: now, updatedAt: now }
  }
  ...
  async addAwemeCache (aweme_id: string, sec_uid: string, groupId: string, pushType: string = 'post'): Promise<AwemeCache> {
    const now = new Date().toISOString()
    await this.runQuery(
      'INSERT OR IGNORE INTO AwemeCaches (aweme_id, sec_uid, groupId, pushType, createdAt, updatedAt) VALUES (?, ?, ?, ?, ?, ?)',
      [aweme_id, sec_uid, groupId, pushType, now, now]
    )

    const cache = await this.getQuery<AwemeCache>(
