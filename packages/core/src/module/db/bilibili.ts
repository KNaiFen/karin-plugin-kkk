...
export class BilibiliDBBase {
  ...
  async getOrCreateBilibiliUser (host_mid: number, remark: string = ''): Promise<BilibiliUser> {
    const now = new Date().toISOString()
    await this.runQuery(
      'INSERT OR IGNORE INTO BilibiliUsers (host_mid, remark, filterMode, createdAt, updatedAt) VALUES (?, ?, ?, ?, ?)',
  ...
  async subscribeBilibiliUser (groupId: string, botId: string, host_mid: number, remark: string = ''): Promise<GroupUserSubscription> {
    ...
    const now = new Date().toISOString()
    await this.runQuery(
      'INSERT OR IGNORE INTO GroupUserSubscriptions (groupId, host_mid, createdAt, updatedAt) VALUES (?, ?, ?, ?)',
    ...
    return subscription || { groupId, host_mid, createdAt: now, updatedAt: now }
  }
  ...
  async addDynamicCache (dynamic_id: string, host_mid: number, groupId: string, dynamic_type: string): Promise<DynamicCache> {
    const now = new Date().toISOString()
    await this.runQuery(
      'INSERT OR IGNORE INTO DynamicCaches (dynamic_id, host_mid, groupId, dynamic_type, createdAt, updatedAt) VALUES (?, ?, ?, ?, ?, ?)',
      [dynamic_id, host_mid, groupId, dynamic_type, now, now]
    )

    const cache = await this.getQuery<DynamicCache>(
