...
export class StatisticsDBBase {
  ...
  async recordParse (groupId: string, userId: string, platform: 'douyin' | 'bilibili' | 'kuaishou' | 'xiaohongshu'): Promise<void> {
    ...
    const groupHadStats = await this.getQuery<{ count: number }>(
      'SELECT COUNT(*) as count FROM ParseStatistics WHERE groupId = ? LIMIT 1',
      [groupId]
    )

    await this.runQuery(
      `INSERT INTO ParseStatistics (groupId, userId, platform, parseCount, createdAt, updatedAt)
       VALUES (?, ?, ?, 1, ?, ?)
       ON CONFLICT(groupId, userId, platform)
       DO UPDATE SET parseCount = ParseStatistics.parseCount + 1, updatedAt = excluded.updatedAt`,
      [groupId, userId, platform, now, now]
    )

    if ((groupHadStats?.count || 0) === 0) {
  ...
  private async updateDailyHistory (date: string, platform: 'douyin' | 'bilibili' | 'kuaishou' | 'xiaohongshu'): Promise<void> {
    ...
    try {
      await this.runQuery(
        `INSERT INTO ParseHistory (date, totalParses, douyin, bilibili, kuaishou, xiaohongshu, createdAt)
         VALUES (?, 1, ?, ?, ?, ?, ?)
         ON CONFLICT(date)
         DO UPDATE SET
           totalParses = ParseHistory.totalParses + 1,
           douyin = ParseHistory.douyin + excluded.douyin,
           bilibili = ParseHistory.bilibili + excluded.bilibili,
           kuaishou = ParseHistory.kuaishou + excluded.kuaishou,
           xiaohongshu = ParseHistory.xiaohongshu + excluded.xiaohongshu`,
