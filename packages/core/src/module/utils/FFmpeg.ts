...
import {
  ...
  type CacheIdentity,
  rememberFileCacheIdentity,
  resolveDerivedArtifact,
  resolveFileCacheIdentity } from './derivedCache'
import { buildBitrateLimitArgs, resolveVideoEncodeArgs, type VideoEncodePreset } from './VideoEncodePreset'
...
export async function fixM4sFile (
  ...
  if (result.status) {
    rememberFileCacheIdentity(outputPath, resolveFileCacheIdentity(inputPath))
...
export const loopVideoWithTransition = async (
  ...
  const derivedCacheIdentity = options.cacheIdentity ?? buildDerivedCacheIdentity(
    'loop-video-with-transition',
    [
      resolveFileCacheIdentity(options.inputPath),
      resolveFileCacheIdentity(options.staticImagePath),
      options.bgmPath ? resolveFileCacheIdentity(options.bgmPath) : { scope: 'derived', key: 'bgm:none' }
    ],
    {
      loopCount: options.loopCount,
      transitionEnabled: options.transitionEnabled ?? true,
      mergeMode: options.mergeMode ?? 'independent',
      usedDuration: options.context?.usedDuration ?? null
    }
  )

  const buildLoopVideo = async (outputPath: string): Promise<{ success: boolean; context?: LiveImageMergeOptions['context'] }> => {
  /** 步骤 1：解析参数与基础配置 */
    const {
      inputPath,
      loopCount,
  ...
  return await resolveDerivedArtifact(
    derivedCacheIdentity,
    options.outputPath,
    async (cacheOutputPath) => await buildLoopVideo(cacheOutputPath)
  )
}
...
export async function mergeVideoAudio (
  ...
  const result = await resolveDerivedArtifact(derivedCacheIdentity, resultPath, async (cacheOutputPath) => {
    const ffmpegResult = await ffmpeg(`-y -i "${videoPath}" -i "${audioPath}" -c copy "${cacheOutputPath}"`)
    if (ffmpegResult.status) {
      logger.debug(`视频合成成功: ${cacheOutputPath}`)
    } else {
      logger.error('视频合成失败', ffmpegResult)
    }
    return {
...
export async function compressVideo (options: CompressVideoOptions): Promise<boolean> {
  ...
  const encodeArgs = await resolveVideoEncodeArgs({
    preset: encodePreset,
    customArgs: customEncodeArgs,
    codec: 'h265'
  })
  const bitrateArgs = buildBitrateLimitArgs(targetBitrate, maxRate, bufSize)
  const result = await ffmpeg(
    `-y -i "${inputPath}" ${bitrateArgs} ${encodeArgs} ` +
    '-vf "scale=\'if(gte(iw/ih,16/9),1280,-1)\':\'if(gte(iw/ih,16/9),-1,720)\',scale=ceil(iw/2)*2:ceil(ih/2)*2" ' +
    `-c:a aac -b:a ${Math.round(audioBitrate)}k -movflags +faststart ` +
    `"${outputPath}"`
  )
