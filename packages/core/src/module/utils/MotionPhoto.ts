...
export const buildGoogleMotionPhoto = async (options: GoogleMotionPhotoOptions): Promise<boolean> => {
  ...
  const derivedCacheIdentity = options.cacheIdentity ?? buildDerivedCacheIdentity(
    'google-motion-photo',
    [
      resolveFileCacheIdentity(imagePath),
      resolveFileCacheIdentity(videoPath)
    ],
    {
      presentationTimestampUs: presentationTimestampUs ?? 0,
      hdrGainMapLength: hdrGainMapLength ?? null,
      livePhotoSystem: Config.app.livePhotoSystem ?? 'google'
    }
  )
  ...
  const result = await resolveDerivedArtifact(derivedCacheIdentity, outputPath, async (cacheOutputPath) => {
    try {
      ...
      fs.mkdirSync(path.dirname(cacheOutputPath), { recursive: true })
      fs.writeFileSync(cacheOutputPath, outputBuffer)

      if (system === 'oppo' && detectedHdrGainMapLength > 0) {
        logger.debug(`Motion Photo 封面生成成功(${system} with HDR): ${cacheOutputPath}`)
      } else {
        logger.debug(`Motion Photo 封面生成成功(${system}): ${cacheOutputPath}`)
      }
      return {
        success: true
      }
