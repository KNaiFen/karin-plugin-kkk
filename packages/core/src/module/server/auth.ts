...
const buildNonceKey = (token: string, nonce: string): string => {
  return crypto.createHash('sha256').update(token).update(':').update(nonce).digest('hex')
}

const cleanupExpiredNonces = (now: number) => {
...
const hasReplayNonce = (token: string, nonce: string, now: number): boolean => {
  cleanupExpiredNonces(now)
  const expiresAt = nonceStore.get(buildNonceKey(token, nonce))
  return typeof expiresAt === 'number' && expiresAt > now
}

const rememberNonce = (token: string, nonce: string, now: number) => {
  nonceStore.set(buildNonceKey(token, nonce), now + NONCE_TTL_MS)
}

const normalizeRequestPathForLog = (requestUrl: string): string => {
  try {
...
export const signatureVerificationMiddleware: RequestHandler = (req, res, next) => {
  try {
    ...
    if (!/^\d+$/.test(timestamp)) {
      return createBadRequestResponse(res, '请求时间戳格式错误')
    }
    ...
    if (timeDiff > NONCE_TTL_MS) { // 5分钟
      return createBadRequestResponse(res, '请求时间戳已过期')
    }

    if (hasReplayNonce(token, nonce, currentTime)) {
      return createBadRequestResponse(res, '请求 nonce 已被使用')
    }
    ...
    if (decodedSignature !== expectedSignature) {
      const path = normalizeRequestPathForLog(url)
      logger.warn(`签名验证失败: method=${method}, path=${JSON.stringify(path)}, reason=signature_mismatch`)
      return createBadRequestResponse(res, '签名验证失败')
    }

    rememberNonce(token, nonce, currentTime)
    next()
  } catch (error: any) {
