...
const getResolveProfile = (platform: SupportedPlatform) => {
  if (platform === 'douyin') return 'douyin-redirect' as const
  if (platform === 'bilibili') return 'bilibili-redirect' as const
  return null
}
...
export const resolveLink: RequestHandler = async (req, res) => {
  try {
    ...
    const inputPlatform = detectPlatform(link)
    const profile = getResolveProfile(inputPlatform)
    if (!profile) {
      return createBadRequestResponse(res, '仅支持抖音或 B 站相关链接解析')
    }

    // 获取重定向后的最终 URL
    const { finalUrl } = await executeSafeAxiosRequest({
      url: link,
      method: 'GET',
      timeout: 15000,
      responseType: 'text',
    ...
    if (platform !== inputPlatform) {
      return createBadRequestResponse(res, '重定向目标平台不在允许范围内')
    }
