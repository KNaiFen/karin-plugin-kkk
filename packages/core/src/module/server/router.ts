...
import { renderVideoPreviewPage } from 'template/server'
...
/**
 * 解析单个 HTTP 字节范围。超出文件末尾的 end 会被截断，无法满足的范围返回 null。
 */
const parseSingleByteRange = (rangeHeader: string, fileSize: number): ByteRange | null => {
  ...
  if (!Number.isSafeInteger(requestedEnd) || start > requestedEnd) {
    return null
  }
...
export const getVideoRouter: RequestHandler = (req, res) => {
  ...
  const htmlContent = renderVideoPreviewPage({
    ...
    resourcePaths: {
      cssDir: path.join(Root.pluginPath, 'lib'),
      imageDir: path.join(Root.pluginPath, 'resources', 'image')
    }
  })
