...
export interface douyinConfig {
  ...
  /** 直播片段录制时长（秒），仅在 sendContent 包含 'video' 且直播中时生效 */
  liveRecordSeconds: number

  /** 直播拉流清晰度，auto 会优先选择最高可用清晰度 */
  liveQuality: 'auto' | 'FULL_HD1' | 'HD1' | 'SD1' | 'SD2'
  ...
  /** 非文章、非直播作品标题超过配置阈值时，独立发送完整标题 */
  longTitleFullText: boolean

  /** 主动解析触发长标题全文输出的 Unicode 字素数阈值 */
  longTitleFullTextThreshold: number
