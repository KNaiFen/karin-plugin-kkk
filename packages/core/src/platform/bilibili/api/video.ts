...
export const getVideoPlayUrl: RequestHandler = async (req, res) => {
  try {
    ...
    if (playUrlData.dash) {
      ...
      for (const video of playUrlData.dash.video.map(preferBilibiliNonMcdnUrls)) {
      ...
      // 处理音频流（按质量排序）
      response.audioStreams = playUrlData.dash.audio
      ...
      response.durlUrl = rewriteBilibiliCdnUrlCarrier(playUrlData.durl[0]).url
