...
export const BilibiliWeb = (all: ConfigType) => {
  const plainTitleReply = normalizePlainTitleReplyConfig(all.bilibili.plainTitleReply, ['video', 'image', 'article', 'live', 'text'])
      ...
      children: [
        components.accordion.createItem('cfg:bilibili', {
          ...
          children: [
            ...
            components.checkbox.group('sendContent', {
              ...
              checkbox: [
                ...
                components.checkbox.create('sendContent:checkbox:3', {
                  label: '视频文件',
                  value: 'video',
                  description: '对视频稿件和直播片段有效'
                })
              ]
            }),
            components.switch.create('plainTitleReply:switch', {
              label: '纯链接补发标题',
              description: '消息内容只有 B站 链接、BV号或 AV号时，解析成功后额外发送「【B站】作者：标题」',
              defaultSelected: plainTitleReply.switch,
              isDisabled: !all.bilibili.switch
            }),
            components.checkbox.group('plainTitleReply:types', {
              label: '补发标题触发类型',
              description: '选择哪些 B站 内容类型在纯链接解析成功后补发标题',
              orientation: 'horizontal',
              defaultValue: plainTitleReply.types,
              isDisabled: !all.bilibili.switch || !plainTitleReply.switch,
              checkbox: [
                ...
                components.checkbox.create('plainTitleReply:types:checkbox:article', {
                  label: '专栏',
                  value: 'article'
                }),
                components.checkbox.create('plainTitleReply:types:checkbox:live', {
                  label: '直播',
                  value: 'live'
                }),
            ...
            components.divider.create('divider-bilibili-live-record', {
              description: '直播录制设置',
              descPosition: 20
            }),
            components.input.number('liveRecordSeconds', {
              label: '直播录制时长',
              description: '解析 B站直播链接时录制直播流片段的时长，单位：秒，仅在发送内容包含「视频文件」时生效',
              defaultValue: all.bilibili.liveRecordSeconds.toString(),
              isDisabled: !all.bilibili.switch || !all.bilibili.sendContent.includes('video'),
              rules: [{ min: 1, max: 120 }]
            }),
            components.radio.group('liveQuality', {
              label: '直播清晰度',
              description: '解析 B站直播链接时请求的直播流清晰度，实际会按接口可用清晰度回退',
              orientation: 'horizontal',
              defaultValue: all.bilibili.liveQuality.toString(),
              isDisabled: !all.bilibili.switch || !all.bilibili.sendContent.includes('video'),
              radio: [
                components.radio.create('liveQuality:radio-1', {
                  label: '流畅',
                  value: '80'
                }),
                components.radio.create('liveQuality:radio-2', {
                  label: '高清',
                  value: '150'
                }),
                components.radio.create('liveQuality:radio-3', {
                  label: '超清',
                  value: '250'
                }),
                components.radio.create('liveQuality:radio-4', {
                  label: '蓝光',
                  value: '400'
                }),
                components.radio.create('liveQuality:radio-5', {
                  label: '原画',
                  value: '10000'
                })
              ]
            ...
            components.input.number('push:jitterSeconds', {
              label: '随机延迟上限（秒）',
              description: 'cron触发后在 0 到该秒数之间随机等待再推送，0 为关闭；设置过大可能导致下一轮任务被跳过',
              defaultValue: String(all.bilibili.push.jitterSeconds ?? 0),
              color: 'warning',
              isDisabled: !all.bilibili.push.switch,
              rules: [{ min: 0, max: 3600 }]
            }),
            components.switch.create('push:riskCooldownEnabled', {
              label: '风控冷却',
              description: '命中B站风控后，自动暂停B站推送与强制推送，避免持续触发风控',
              defaultSelected: all.bilibili.push.riskCooldownEnabled ?? true,
              color: 'warning',
              isDisabled: !all.bilibili.push.switch
            }),
            components.input.number('push:riskCooldownMs', {
              label: '风控冷却时长（毫秒）',
              description: '命中 B站 风控后，本轮起暂停多久再恢复推送',
              defaultValue: String(all.bilibili.push.riskCooldownMs ?? 3600000),
              color: 'warning',
              isDisabled: !all.bilibili.push.switch || !(all.bilibili.push.riskCooldownEnabled ?? true),
