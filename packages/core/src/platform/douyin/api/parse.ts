...
export const parseWork: RequestHandler = async (req, res) => {
  try {
    ...
    // 并发获取作品数据、评论数据、表情数据
    const [workFetchResult, commentsResponse, emojiResponse] = await Promise.all([
      fetchDouyinOneWork({ type: 'one_work', aweme_id }),
      douyinFetcher.fetchWorkComments({ aweme_id, typeMode: 'strict' }),
      douyinFetcher.fetchEmojiList({ typeMode: 'strict' })
    ])
    
    const awemeDetail = (workFetchResult.workData.data as DyVideoWork | DyImageAlbumWork | DySlidesWork).aweme_detail
    ...
    if (isSlides && awemeDetail.images) {
      slides = awemeDetail.images.map((item) => {
        if (item.clip_type === 2) {
          ...
          const videoUrl = getDouyinPlayableVideoUrl(item.video)
          return {
    ...
    const workInfo: DouyinWorkInfo = {
      ...
      downloadUrl: {
        video: isVideo
          ? getDouyinPlayableVideoUrl(awemeDetail.video)
          : isSlides && slides
