...
type Snapka = typeof import('@karinjs/plugin-puppeteer')['snapka']
type Page = Awaited<ReturnType<Awaited<ReturnType<Snapka['launch']>>['browser']['newPage']>>
...
export const douyinLogin = async (e: Message) => {
  const msg_id: string[] = []
  const { snapka } = await import('@karinjs/plugin-puppeteer')
