...
async function getEncoderParams (
  ...
  if (targetBitrate && targetBitrate > 0) {
    ...
    return `${buildBitrateLimitArgs(adjustedBitrate, adjustedBitrate * 2.5, adjustedBitrate * 4)} ${encodeArgs}`
  }
...
export async function burnDouyinDanmaku (
  ...
  const result = await resolveDerivedArtifact(derivedCacheIdentity, outputPath, async (cacheOutputPath) => {
    ...
    if (ffmpegResult.status) {
      logger.mark(`[DouyinDanmaku] 弹幕烧录成功: ${cacheOutputPath}`)
      if (removeSource) Common.removeFile(videoPath)
    } else {
      logger.error('[DouyinDanmaku] 弹幕烧录失败', ffmpegResult)
    }
