...
export const XiaohongshuWeb = (all: ConfigType) => {
  const plainTitleReply = normalizePlainTitleReplyConfig(all.xiaohongshu.plainTitleReply, ['video', 'image'])
        ...
        components.accordion.createItem('cfg:xiaohongshu', {
          ...
          children: [
            ...
            components.switch.create('plainTitleReply:switch', {
              label: '纯链接补发标题',
              description: '消息内容只有小红书链接时，解析成功后额外发送「【小红书】作者：标题」',
              defaultSelected: plainTitleReply.switch,
              isDisabled: !all.xiaohongshu.switch
            }),
            components.checkbox.group('plainTitleReply:types', {
              label: '补发标题触发类型',
              description: '选择哪些小红书内容类型在纯链接解析成功后补发标题',
              orientation: 'horizontal',
              defaultValue: plainTitleReply.types,
              isDisabled: !all.xiaohongshu.switch || !plainTitleReply.switch,
              checkbox: [
