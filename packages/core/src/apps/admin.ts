...
import { getSharedCacheTtlMs, removeExpiredFilesRecursively } from '@/module/utils/sharedCache'
import { bilibiliLogin } from '@/platform'
import { douyinLogin } from '@/platform/douyin/login'

// 包装缓存清理任务
const handleTempCacheCleanup = wrapWithErrorHandler(async () => {
...
const handleSharedCacheCleanup = wrapWithErrorHandler(async () => {
  const beforeTimestamp = Date.now() - getSharedCacheTtlMs()
  const cacheDirectories = [
    Common.tempDri.cache.parsedPost,
    Common.tempDri.cache.workBundle,
    Common.tempDri.cache.media,
    Common.tempDri.cache.renderAssets,
    Common.tempDri.cache.derived
  ]

  for (const dir of cacheDirectories) {
    const deleted = removeExpiredFilesRecursively(dir, beforeTimestamp)
    logger.debug(`${dir} 目录下已删除 ${deleted} 个共享缓存文件`)
  }
}, {
  businessName: '共享缓存自动删除'
})

export const task = Config.app.removeCache
  ? karin.task('[kkk-缓存自动删除]', '*/30 * * * *', handleTempCacheCleanup, { log: false })
  : null

export const sharedCacheTask = karin.task('[kkk-共享缓存自动删除]', '17 */1 * * *', handleSharedCacheCleanup, { log: false })
