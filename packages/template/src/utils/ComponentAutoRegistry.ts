 ...
 * 组件注册表接口
 */
type RegisteredComponentProps = never

interface ComponentRegistryItem {
  /** 组件构造函数 */
  component: React.ComponentType<RegisteredComponentProps>
  /** 数据类型验证函数 */
...
export class ComponentAutoRegistry {
  ...
  private static async registerComponent(platform: string, componentConfig: ComponentConfig): Promise<void> {
    ...
    if (componentConfig.component) {
      ...
      this.components.set(key, {
        component: module.default as React.ComponentType<RegisteredComponentProps>,
        validateData: componentConfig.validateData,
        config: componentConfig
      })
    } else {
      ...
      const component = module[componentConfig.exportName] as React.ComponentType<RegisteredComponentProps> | undefined
