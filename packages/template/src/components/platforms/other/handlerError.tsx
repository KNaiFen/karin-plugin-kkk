...
import type { ApiErrorProps, BusinessError, LogLevel } from '@kkk/template-contracts'
...
const getAdapterVersionLabel = (version: unknown): string => {
  const normalized = getSafeText(version)
  if (!normalized) return '未知版本'
  return normalized.startsWith('v') ? normalized : `v${normalized}`
}

const getAdapterLogo = (adapterName: unknown): React.ReactNode => {
  const normalizedName = getSafeText(adapterName)
  const nameLower = normalizedName.toLowerCase()
  for (const [key, logoPath] of Object.entries(ADAPTER_LOGO_MAP)) {
    if (nameLower.includes(key)) return <img src={logoPath} className='h-20 w-auto' alt={normalizedName || 'Adapter'} />
  }
...
export const handlerError: React.FC<Omit<ApiErrorProps, 'templateType' | 'templateName'>> = (props) => {
  ...
  const adapterName = getDisplayText(data.adapterInfo?.name, '未知适配器')
  const adapterVersion = getAdapterVersionLabel(data.adapterInfo?.version)
  const adapterPlatform = getDisplayText(data.adapterInfo?.platform, '未知平台')
  const adapterStandardRaw = getSafeText(data.adapterInfo?.standard)
  const adapterStandard = adapterStandardRaw.toLowerCase()
  const adapterStandardDisplay = getDisplayText(_.upperFirst(_.camelCase(adapterStandardRaw)), '未知标准')
  const adapterProtocol = getDisplayText(data.adapterInfo?.protocol, '未知协议实现')
  const adapterCommunication = getDisplayText(data.adapterInfo?.communication, '未知通信方式')
          ...
          <div className='flex items-center gap-8 pr-12'>
            ...
            <div className='text-right'>
              <div className='flex items-center justify-end gap-3 mb-1'>
                <span className='text-xs font-black tracking-[0.3em] uppercase opacity-60' style={{ color: mutedColor }}>Trigger Time</span>
              ...
              >
                  ...
                  <div className='flex items-center gap-6 min-w-0'>
                    ...
                    <div className='min-w-0'>
                      ...
                      <div className='flex items-center gap-4 flex-wrap'>
                        <p className='text-3xl font-bold truncate' style={{ color: accentColor }}>{adapterName}</p>
                ...
                <div className='grid grid-cols-4 gap-4 text-lg' style={{ color: secondaryColor }}>
                  <div className='rounded-2xl px-4 py-3' style={{ backgroundColor: isDark ? 'rgba(248,113,113,0.08)' : 'rgba(220,38,38,0.05)' }}>
                    ...
                    <p className='font-semibold break-all text-2xl'>{adapterPlatform}</p>
                  </div>
                  <div className='rounded-2xl px-4 py-3 relative overflow-hidden' style={{ backgroundColor: isDark ? 'rgba(248,113,113,0.08)' : 'rgba(220,38,38,0.05)' }}>
                    ...
                    <p className='font-semibold break-all text-2xl'>{adapterStandardDisplay}</p>
                    {adapterStandard === 'milky' && (
                      <div className='absolute inset-0 pointer-events-none'>
                        <img
                    ...
                    {adapterStandard.includes('onebot') && (
                      <div className='absolute inset-0 pointer-events-none'>
                        <img
                  ...
                  <div className='rounded-2xl px-4 py-3' style={{ backgroundColor: isDark ? 'rgba(248,113,113,0.08)' : 'rgba(220,38,38,0.05)' }}>
                    ...
                    <p className='font-semibold break-all text-2xl'>{adapterProtocol}</p>
                  </div>
                  <div className='rounded-2xl px-4 py-3' style={{ backgroundColor: isDark ? 'rgba(248,113,113,0.08)' : 'rgba(220,38,38,0.05)' }}>
                    <p className='text-sm mb-1 opacity-75'>Communication / 通信方式</p>
                    <p className='font-semibold break-all text-2xl'>{adapterCommunication}</p>
                  </div>
                </div>
              </div>
            )}
