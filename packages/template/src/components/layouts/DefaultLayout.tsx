...
export const DefaultLayout: React.FC<Omit<DefaultLayoutProps, 'templateType' | 'templateName'>> = ({
  ...
  const useDarkTheme = 'useDarkTheme' in data && data.useDarkTheme === true
  ...
  return (
    ...
    >
      {children}
      {version ? (
        <div className='relative z-50 pt-32 pb-20 text-foreground/80' data-page-avoid-split>
          {/* 版本信息：插件、框架、构建工具 */}
          ...
          {typeof watermarkTextBitSize === 'number' && (
            <div className='flex justify-center' data-page-block>
              <span className='text-xs font-mono text-foreground/30'>Restore ID: {watermarkTextBitSize}</span>
            </div>
          )}
        </div>
      ) : (
        <div className='flex items-center justify-center h-24' data-page-avoid-split>
          {typeof watermarkTextBitSize === 'number' && (
            <span className='text-xs font-mono text-foreground/30'>Restore ID: {watermarkTextBitSize}</span>
          )}
        </div>
