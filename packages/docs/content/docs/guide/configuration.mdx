...
| 插件应用相关 | 全局行为 | 默认解析（`videoTool`）、渲染主题（`Theme`）、Live Photo、错误日志 |
...
插件对外只保留一套平台 HTTP API 面：`/api/kkk/v1`。

- B 站平台接口统一位于 `/api/kkk/v1/platforms/bilibili/*`
- 抖音平台接口统一位于 `/api/kkk/v1/platforms/douyin/*`
- 鉴权和签名校验继续沿用 `v1` 现有规则

旧的嵌入式 `amagi` HTTP 路由与独立 API 服务能力已经移除，不再提供：

- `/api/kkk/amagi/api/*`
- 独立端口上的 `/api/bilibili/*`
- 独立端口上的 `/api/douyin/*`
- 独立端口上的 `/api/kuaishou/*`
- 独立端口上的 `/api/xiaohongshu/*`
...
- 智能分页渲染（`multiPageRender`）：长图内容很多时建议保持开启
- 分页触发比例 / 分页后单页比例（`multiPageTriggerAspectRatio` / `multiPageMaxAspectRatio`）：控制超长渲染图何时分页、每页最多多高
- 渲染图输出格式 / JPG 质量（`renderImageFormat` / `renderImageQuality`）：控制是自动推荐、强制 JPG 还是强制 PNG
